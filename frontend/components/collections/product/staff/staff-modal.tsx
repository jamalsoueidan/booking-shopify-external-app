...
import { usePosition, useProductStaff, useTag, useTranslation } from "@jamalsoueidan/pkg.frontend";
...
export default ({ show, close }: StaffModalProps) => {
  ...
  const didChange = useMemo(() => JSON.stringify(value) !== JSON.stringify(selected), [value, selected]);
  ...
  const choices = useMemo(() => {
    ...
    return sorted.map((staff) => <ChoiceStaff key={staff._id} staff={staff} toggle={toggle} selected={selected} />);
...
const ChoiceStaff = ({ staff, selected, toggle }: ChoiceStaffProps) => {
  ...
  const choiceSelected = selected.filter((s) => s._id === staff._id).map((s) => s.tag);
